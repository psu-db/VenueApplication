...
namespace VenueApplication.Services
{
    internal static class LoginService
    {
        ...
        public static (bool, user_wallet?, login_credentials?, app_user?) AttemptLogin(string username, string inputPassword, DatabaseManager databaseManager)
        {
            (bool attemptGetHashedPassResult, string? hashedPassword) = AttemptGetHashedPassword(username, databaseManager);

            if (attemptGetHashedPassResult)
            {
                ...
                using (var dbConnection = databaseManager.GetConnection())
                {
                    ...
                    var verificationResult = hasher.VerifyHashedPassword(null, hashedPassword, inputPassword);

                    if (verificationResult == PasswordVerificationResult.Success)
                    {
                        Debug.WriteLine("Login successful!, running login query to fetch necessary startup metadata");
        ...
        public static (bool, string?) AttemptGetHashedPassword(string username, DatabaseManager databaseManager)
        {
            //this is the local branch
            string query = VenueApplication.Properties.Resource.userGetHashedPassword_SELECT;
                    ...
                    using (var reader = command.ExecuteReader())
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (reader.Read())
                            {
                                ...
                                string? hashedPassword = reader.IsDBNull(reader.GetOrdinal("lgn_password")) ? null : reader.GetString(reader.GetOrdinal("lgn_password"));
                                
                                Debug.WriteLine("Found hashed pasword given the username");
                        ...
                        {
                            Debug.WriteLine("Attempt to find hashed password given username failed.");
                            return (false, null);
                        }
                    }
                }
